...
const size_t AMOUNT_POSSIBLE_NUMBERS = MAX_NUMBER - MIN_NUMBER + 1;
 ...
 * O(M+N) Memory
 */
void fill_and_check_occurrence(int * array, int * occurrence_map, size_t size, int min, int max) {
    srand(time(NULL));
    size_t i; int ran_num;
    for (i = 0; i < size; i++) {
