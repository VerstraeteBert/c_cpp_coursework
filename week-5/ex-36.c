...
double euclid_dist(const struct point * one, const struct point * two) {
    return sqrt(pow(one->x - two->x, 2) + pow(one->y - two->y, 2));
}

double manhattan_dist(const struct point * one, const struct point * two) {
    return fabs(one->x - two->x) + fabs(one->y - one->y);
}

double chebyshev_dist(const struct point * one, const struct point * two) {
    double x_diff = fabs(one->x - two->x);
    double y_diff = fabs(one->y - two->y);
    return x_diff > y_diff ? x_diff : y_diff;
}

double calc_max_dist(const struct point * points, size_t size, double(* diff)(const struct point *, const struct point *)) {
    double curr_max = -1;
